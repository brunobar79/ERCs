---
eip: 8050
title: Compressed RPC Link Format
description: A compact, URL-safe format for wallet RPC requests using Protocol Buffers and Base64url encoding.
...
- **Shortcut 1**: `wallet_sendCalls` ([EIP-5792](./eip-5792.md)) with optimized encodings for [ERC-20](./eip-20.md) transfers, native transfers, and generic calls
- **Shortcut 2**: `wallet_sign` ([EIP-7871](./eip-7871.md)) with optimized encodings for spend permissions and receive-with-authorization signatures
...
Payloads are Base64url-encoded ([RFC 4648](https://www.rfc-editor.org/rfc/rfc4648#section-5)) binary data without padding. The entire string is the payload; applications MAY wrap it in URLs.
...
- Decoders MUST implement Brotli decompression for `0x01` with support for:
  ...
  - Standard Brotli [RFC 7932](https://www.rfc-editor.org/rfc/rfc7932) format
...
1. **ERC-20 Transfer**: Exactly one call where:
   - `data` starts with `0xa9059cbb` (ERC-20 `transfer(address,uint256)` selector)
...
3. Extract fields:
   - **ERC-20**: Extract `token` (to address), `recipient` (bytes 4-35 of data), `amount` (bytes 36-67 of data, decoded as uint256)
...
**Purpose:** Optimized encoding for EIP-7871 `wallet_sign` requests with [EIP-712](./eip-712.md) typed data.
...
2. **Select shortcut**:
   - `wallet_sendCalls` → shortcut 1 (detect type: ERC-20/Native/Generic)
...
The `dataCallback` capability enables wallet→server event notifications for enhanced UX and allows apps to request user data. It is defined in ERC-8026.
...
|------------------|-----------------|------------------------|----------------------------|
| ERC-20 Transfer  | ~280-350        | ~120-150               | ~90-120                    |
...
#### Test Vector 1: ERC-20 Transfer (Shortcut 1)
